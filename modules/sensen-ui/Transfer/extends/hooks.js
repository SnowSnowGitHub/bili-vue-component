...
export function useComputedData(data, tartgetIndex, rightListData, checkedData) {
  const leftTitle = computed(() => data[tartgetIndex.value].title)

  const leftListData = computed(() => {
    const { data: currentData } = data[tartgetIndex.value]
